...
# This starts the python implementation of the docker_db_starter.
# It tries to discover if pipenv is already installed or tries to install it if it not exists.
# After that it creates the virtual environment and install all necessary dependencies for docker-db-starter.
# In the end it runs docker_db_starter_src/docker_db_starter.py in the virtual environment.
