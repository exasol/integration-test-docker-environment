...
class StillRunningLogger():
    ...
    def log(self, message=None):
        ...
        if self._previous_time + timedelta_between <= datetime.now():
            if message is None:
                self._logger.info("Task %s: Still running %s.", self._taskid, self._description)
            else:
                self._logger.info("Task %s: Still running %s. Message: %s", self._taskid, self._description, message)
            self._previous_time = datetime.now()
