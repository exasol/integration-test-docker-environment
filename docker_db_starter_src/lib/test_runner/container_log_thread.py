...
from docker_db_starter_src.lib.container_log_handler import ContainerLogHandler
...
class ContainerLogThread(Thread):
    ...
    def stop(self):
        self.logger.info("Task %s: Stop ContainerLogThread", self.task_id)
        self.finish = True
        ...
        with ContainerLogHandler(self.log_file, self.logger, self.task_id, self.description) as log_handler:
            ...
            while not self.finish:
                ...
                if "error" in log_line or "exception" in log_line or "returned with state 1" in log_line:
                    self.logger.info("Task %s: ContainerLogHandler error message, %s", self.task_id, log_line)
