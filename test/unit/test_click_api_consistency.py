...
def test_api_arguments(check):
    ...
    for cli_call, api_call in zip(click_commands, api_functions):
        ...
        with check:
            assert api_spec.args == cli_spec.args, f"{loop} - cli spec"
        with check:
            assert (
                api_spec.annotations == cli_spec.annotations
            ), f"{loop} - type annotations"
        with check:
            assert api_spec.args == param_names_of_click_call(
                cli_call
            ), f"{loop} - cli param names"
...
def test_api_default_values():
    ...
    for cli_call, api_call in zip(click_commands, api_functions):
        ...
        assert len(cli_defaults) == len(api_spec_defaults)
