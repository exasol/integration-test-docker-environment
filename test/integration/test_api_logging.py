...
def dummy_api_command(
    log_level: Optional[str], use_job_specific_log_file: bool
):
...
def assert_loggers_are_equal(logger_infos_after, logger_infos_before):
    ...
    assert logger_infos_before[API_CLIENT_LOGGING_TEST_LOGGER] == logger_infos_after[API_CLIENT_LOGGING_TEST_LOGGER]
    assert logger_infos_before[LUIGI_LOGGER] == logger_infos_after[LUIGI_LOGGER]
    assert logger_infos_before[LUIGI_INTERFACE_LOGGER] == logger_infos_after[LUIGI_INTERFACE_LOGGER]
...
def test_luigi_log_level_info_and_basic_logging_error(capfd, custom_logging, luigi_output):
    """
    ...
    with custom_logging(log_level=logging.ERROR) as error_log:
        ...
        assert re.compile(".*===== Luigi Execution Summary =====.*").search(stderr_output) is not None

        main_log_glob = list(
            luigi_output.glob("**/main.log")
        )
        assert main_log_glob == []


def test_luigi_log_level_error_and_basic_logging_info(capfd, custom_logging, luigi_output):
    """
...
def test_luigi_log_level_error_multiple_calls_and_basic_logging_info(capfd, custom_logging, luigi_output):
    """
    This test checks if setting luigi log level to level ERROR and global log level to INFO
...
def test_luigi_use_job_specific_log_file_and_basic_logging_error(capfd, custom_logging, luigi_output):
    """
    ...
    with custom_logging(log_level=logging.ERROR) as error_log:
        ...
        if sys.version_info[1] < 12:
            ...
            assert " is multi-threaded, use of fork() may lead to deadlocks in the child" in stderr_output_lines[0]
...
def test_luigi_no_log_config_and_basic_logging_info(capfd, custom_logging, luigi_output):
    """
...
def test_luigi_no_log_config_and_basic_logging_error(capfd, custom_logging, luigi_output):
    """
...
def create_logger_infos() -> Dict[str, Dict[str, Any]]:
    logger_infos = {
        ...
        LUIGI_INTERFACE_LOGGER: get_logger_info(
