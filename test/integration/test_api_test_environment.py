...
def test_docker_available_in_test_container(api_default_database_with_test_conainer_module):
...
def test_db_container_available(api_default_database_with_test_conainer_module):
    ...
    with ContextDockerClient() as docker_client:
        ...
        exit_code = exit_result[0]
        output = exit_result[1]
        assert exit_code == 0, f"Error while executing 'ls /exa' in db container got output\n {output}."
...
def test_db_available(api_default_database_with_test_conainer_module):
    ...
    with ContextDockerClient() as docker_client:
        ...
        exit_result = test_container.exec_run(create_db_connection_command(test_environment))
        exit_code = exit_result[0]
        output = exit_result[1]
        assert exit_code == 0, f"Error while executing 'exaplus' in test container got output\n {output}."
...
def create_db_connection_command(test_environment):
    ...
    db_port = (
        test_environment.environment_info.database_info.ports.database
    )
