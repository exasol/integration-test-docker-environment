...
ADDITIONAL_DB_PARAMS = [
    "-disableIndexIteratorScan=1",
    "-disableIndexIteratorScan=1",
    "-docker-access-method=DOCKER_EXEC"]
...
class DockerTestEnvironmentAdditionParamsTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        additional_db_parameters = [f"--additional-db-parameter {add_param}" for add_param in ADDITIONAL_DB_PARAMS]
    ...
    def test_additional_params_are_used(self):
        ...
        with ContextDockerClient() as docker_client:
            ...
            params_lines = [line for line in output.decode("UTF-8").splitlines() if line.startswith("Parameters:")]
            self.assertEqual(len(params_lines), 1, "Unexpected format of output of dwad_client")
