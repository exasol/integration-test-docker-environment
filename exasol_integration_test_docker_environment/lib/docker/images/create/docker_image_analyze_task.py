...
class DockerAnalyzeImageTask(DockerBaseTask):
    ...
    def register_required(self):
        ...
        if task_classes is not None:
            if (
                not isinstance(task_classes, dict)
                or not self.keys_are_string(task_classes)
                or not self.values_are_subclass_of_baseclass(task_classes)
            ):
                raise TypeError(
                    f"Expected Dict[str,DockerAnalyzeImageTask] got {task_classes}"
                )
            tasks = {
                key: self.create_child_task_with_common_params(value)
    ...
    def keys_are_string(self, task_classes) -> bool:
        return all(isinstance(key, str) for key in task_classes.keys())
    ...
    def run_task(self):
        ...
        image_state = self.get_image_state(
            source_image_target, target_image_target, image_info_of_dependencies
        )
    ...
    def get_commit_id(self):
        try:
            ...
            self.logger.info(
                "Not a Git Repository, can't determine the commit id for the image_info"
            )
            return ""
    ...
    def can_image_be_loaded(self, image_target: DockerImageTarget):
        if build_config().cache_directory is not None:
            ...
            self.logger.info(
                f"Checking if image archive {image_path} "
    ...
    def get_path_to_cached_image(self, image_target):
        image_path = Path(build_config().cache_directory).joinpath(
            Path(image_target.get_complete_name() + ".tar")
        )
        return image_path
    ...
    def is_image_in_registry(self, image_target: DockerImageTarget):
        try:
            self.logger.info(
                "Try to find image %s in registry", image_target.get_complete_name()
            )
            ...
            if exists:
                self.logger.info(
                    "Found image %s in registry", image_target.get_complete_name()
                )
            return exists
        except docker.errors.DockerException as e:
            self.logger.warning(
                "Image %s not in registry, got exception %s",
                image_target.get_complete_name(),
                e,
            )
            return False
