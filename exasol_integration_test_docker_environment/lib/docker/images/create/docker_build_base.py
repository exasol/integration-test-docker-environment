...
class DockerBuildBase(DependencyLoggerBaseTask):
    ...
    def register_required(self):
        self.analyze_tasks_futures = self.register_dependencies(
    ...
    def _create_build_task_for_image_info(
        self, image_info: ImageInfo, shortcut_build: bool = True
    ) -> DockerCreateImageTask:
        if self._build_with_depenencies_is_requested(image_info, shortcut_build):
            task_for_image_info = self._create_build_task_with_dependencies(
                image_info, shortcut_build
            )
            return task_for_image_info
        else:
            image_name = f"{image_info.target_repository_name}:{image_info.target_tag}"
            task_for_image_info = self.create_child_task(
                DockerCreateImageTask, image_name=image_name, image_info=image_info
            )
            return task_for_image_info

    def _build_with_depenencies_is_requested(
        self, image_info: ImageInfo, shortcut_build: bool
    ):
        needs_to_be_build = image_info.image_state == ImageState.NEEDS_TO_BE_BUILD.name
        result = (
            (not shortcut_build or needs_to_be_build)
            and image_info.depends_on_images
            and len(image_info.depends_on_images) > 0
        )
        return result
    ...
    def _create_build_task_with_dependencies(
        ...
        required_tasks = self._create_build_tasks_for_image_infos(
    ...
    def _create_required_task_info(
        self, required_task: DockerCreateImageTask
    ) -> RequiredTaskInfo:
        required_task_info = RequiredTaskInfo(
            module_name=required_task.__module__,
