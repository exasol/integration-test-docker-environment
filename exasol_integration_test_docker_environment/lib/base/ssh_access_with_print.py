...
class SshKey:
    ...
    def from_folder(cls, folder: Optional[Path] = None, args=None) -> 'SshKey':
        ...
        if args:
            print(f'{args.id}: waiting for exclusive access...')
        with portalocker.Lock(_path(_LOCK_FILE), 'wb', timeout=10) as fh:
            if args and args.sleep > 0:
                print(f"process {args.id}: occupying exclusive access for {args.sleep} seconds...")
...
def connect_to_vm(key: SshKey):
    connect_kwargs = { "pkey": key.private }
    result = Connection('192.168.1.2', user="chr", connect_kwargs=connect_kwargs).run('uname -a')
...
def arg_parser():
    ...
    parser.add_argument(
        "-s",
        "--sleep",
        help="Seconds to sleep before entering semaphore",
        type=int,
    )
    parser.add_argument(
        "--id",
        help="Id of current process",
    )
    return parser
...
def main():
    ...
    key = SshKey.from_folder(SshKey.default_folder(), args)
    connect_to_vm(key)
